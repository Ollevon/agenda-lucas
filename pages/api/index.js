...
async function askAI(text, commitments) {
  ...
  const system = `Agenda de audiovisual. HOJE=${today}(${weekday}). Responda APENAS JSON compacto sem markdown.

DATAS: resolva datas relativas para YYYY-MM-DD. "datas" é array (vários dias possíveis). Sem data=[].
CATEGORIA: "trabalho"=captação/edição/render/entrega/gravação/color; "reuniao"=calls/briefing/alinhamento; "pessoal"=médico/academia/família/pessoal.
STATUS: "incerto"=talvez/pode ser/a confirmar; "em_andamento"=em curso; "concluido"=entregue; "confirmado"=padrão.
TIPO: captacao|edicao|producao|reuniao|entrega|pessoal|outro.
PRIORIDADE: alta(urgente)|media(padrão)|baixa.
CLIENTE: só para trabalho/reunião com cliente.
CONFLITO: se trabalho confirmado cair num dia com outro trabalho confirmado, mencione na reply (1 frase). Reunião/pessoal nunca conflitam.

Agenda atual: [${ctx || "vazia"}].

Formato obrigatório (JSON minificado, reply máx 1 frase):
{"reply":"","operations":[{"type":"add|update|complete|confirm|delete","id":null,"commitment":{"titulo":"","cliente":"","categoria":"","tipo":"","datas":[],"hora":null,"status":"","prioridade":"","notas":""}}]}

Para "complete"→status=concluido; "confirm"→status=confirmado; "delete"→commitment={}. Operations=[] se não for tarefa.`;
