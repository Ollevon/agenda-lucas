...
.ag-chat{width:340px; flex-shrink:0; border-left:1px solid var(--line); display:flex; flex-direction:column; background:var(--panel)}
...
.job-meta{display:flex; align-items:center; gap:12px; margin-top:9px}
...
/* modal do dia */
.modal-bk{position:fixed; inset:0; z-index:90; background:rgba(6,7,9,.66); backdrop-filter:blur(2px); display:flex; align-items:center; justify-content:center; padding:24px; animation:fade .18s ease both}
@keyframes fade{from{opacity:0} to{opacity:1}}
.modal{width:100%; max-width:520px; max-height:80vh; background:var(--panel); border:1px solid var(--line); border-radius:16px; display:flex; flex-direction:column; overflow:hidden; box-shadow:0 24px 60px -16px rgba(0,0,0,.7); animation:rise .22s ease both}
...
@media (max-width:860px){
  ...
  .ag-chat{width:100%; height:44vh; border-left:none; border-top:1px solid var(--line)}
...
function datesOf(c) {
  if (Array.isArray(c.datas) && c.datas.length) return c.datas;
...
function normDates(arr) {
  return [...new Set((arr || []).filter(d => ISO_RE.test(d)))].sort();
}
...
async function askAI(text, commitments) {
  ...
  const ctx = commitments.map(c =>
    `{id:"${c.id}", titulo:"${c.titulo}", cliente:"${c.cliente || ""}", categoria:"${c.categoria || ""}", status:"${c.status}", datas:[${datesOf(c).join(",")}]}`
  ).join(", ");

  const system = `Você é o motor de uma agenda para um profissional de audiovisual (captação, edição e produção de vídeo) no Brasil. Interpreta mensagens em português e converte em operações sobre a agenda.

HOJE é ${today} (${weekday}). Resolva datas relativas ("amanhã", "sexta", "semana que vem", "dia 15", "fim do mês") para o futuro mais próximo no formato YYYY-MM-DD.

DATAS: o campo "datas" é uma LISTA de datas (YYYY-MM-DD). Um mesmo trabalho pode ter VÁRIOS dias — ex: "captação dias 12, 13 e 14" ou "gravação de terça a quinta" → liste todas as datas. Se for um dia só, liste uma data. Se não houver data, use [].

CATEGORIA (sempre defina uma):
- "trabalho" → produção audiovisual: captação, edição, render, entrega, gravação, decupagem, color, trilha.
- "reuniao" → reuniões, calls, alinhamentos, briefings, orçamentos, conversas com cliente.
- "pessoal" → vida pessoal: médico, dentista, academia, família, aniversário, contas, mercado, viagem pessoal, descanso.

STATUS:
- "incerto" → quando há incerteza: "talvez", "pode ser", "a confirmar", "não sei se", "se rolar", "incerto", "provável".
- "em_andamento" → trabalho já começado/em curso.
- "concluido" → finalizado/entregue/pronto.
- "confirmado" → padrão para um compromisso firme sem incerteza.

TIPO: captacao | edicao | producao | reuniao | entrega | pessoal | outro (coerente com a categoria).
PRIORIDADE: alta | media | baixa (alta só se houver urgência clara; padrão media).
CLIENTE: só quando fizer sentido (categoria trabalho ou reunião com cliente). Para pessoal, deixe vazio.

CONFLITO: se a mensagem agendar/confirmar um TRABALHO num dia que já tem outro trabalho confirmado na agenda atual, mencione na reply e sugira mover, marcar como incerto ou manter. Reuniões e pessoal podem se sobrepor sem problema.

A agenda atual é: [${ctx || "vazia"}].
Se a mensagem se referir a um item existente (concluir, confirmar, remarcar, adicionar/tirar uma data, mudar status/categoria, excluir), use o id correspondente.

Responda SOMENTE com JSON válido, sem markdown, sem crases, neste formato:
{"reply":"confirmação curta e natural em pt-BR","operations":[{"type":"add|update|complete|confirm|delete","id":"id existente ou null","commitment":{"titulo":"","cliente":"","categoria":"","tipo":"","datas":["YYYY-MM-DD"],"hora":"HH:MM ou null","status":"","prioridade":"","notas":""}}]}

Para "complete" use status concluido; para "confirm" use status confirmado. Para "delete" o commitment pode ser {}. Pode haver múltiplas operações. Se a mensagem não for uma tarefa, retorne operations vazio e responda normalmente.

IMPORTANTE: seja MUITO conciso. O campo "reply" deve ter no máximo uma frase curta. Gere o JSON compacto, sem espaços ou quebras de linha extras, pra caber no limite de resposta.`;
  ...
  catch { throw new Error(`HTTP ${res.status} — resposta inesperada do servidor.`); }

  if (!res.ok || data?.error) {
    throw new Error(data?.error || `Erro ${res.status} ao falar com a IA.`);
  }

  let raw = (data.text || "").replace(/```json/gi, "").replace(/```/g, "").trim();
  const a = raw.indexOf("{"), z = raw.lastIndexOf("}");
  if (a === -1 || z === -1) throw new Error(`Sem JSON na resposta: ${raw.slice(0, 120)}`);
  try {
    return JSON.parse(raw.slice(a, z + 1));
  } catch {
    throw new Error(`JSON inválido na resposta: ${raw.slice(0, 120)}`);
  }
}
...
export default function App() {
  ...
  const [messages, setMessages] = useState([
    { role: "bot", text: "Manda os seus compromissos do jeito que vierem na cabeça — eu organizo. },
  ]);
    ...
    fetch("/api/items")
      ...
      .then(data => {
        if (Array.isArray(data.items) && data.items.length) {
          const arr = data.items.map(c => {
            const datas = normDates(c.datas && c.datas.length ? c.datas : (c.data ? [c.data] : []));
            return { ...c, categoria: inferCategoria(c), datas, data: datas[0] || null };
          });
          setItems(arr);
        }
      })
  ...
  const catOf = (c) => c.categoria || inferCategoria(c);
  ...
  const renderJob = (c, ctxDay) => {
    const st = STATUS[c.status]; const kat = CATEGORIA[catOf(c)];
  ...
  return (
    <div className="ag-root">
      ...
      {/* CHAT / CONSOLE */}
      <div className={"ag-chat" + (collapsed ? " collapsed" : "")}>
        ...
        <div className="modal-bk" onClick={() => setSelDay(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-h">
              <div>
                <div className="modal-d disp">{(() => { const d = parseISO(selDay); return `${WD[d.getDay()]}, ${d.getDate()} de ${MONTHS_FULL[d.getMonth()].toLowerCase()}`; })()}</div>
                <div className="modal-s">{(eventsByDay[selDay] || []).length} compromisso(s)</div>
              </div>
            ...
            <div className="modal-body scroll">
              {isConflictDay(selDay) && (
                <div className="modal-warn">
                  <strong>⚠ Mais de um trabalho confirmado neste dia.</strong> Decida: mova um pra outra data, marque um como <em>incerto</em> pra estacionar, ou mantenha os dois se for proposital. Reuniões e compromissos pessoais não entram nessa conta.
                </div>
              )}
